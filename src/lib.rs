...
pub extern fn file_creation_date(path: *const i8) -> f64 {
  ...
  creation_time.duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() as f64
}
...
pub extern fn file_modification_date(path: *const i8) -> f64 {
  ...
  modification_time.duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() as f64
}
...
pub extern fn file_access_date(path: *const i8) -> f64 {
  ...
  access_time.duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() as f64
}
